...
- Root cause:
  - Availability logic only subtracted already-booked slots and never removed already-passed slots for today; client UI trusted backend slot list without same-day guard.
- Files changed:
  - `apps/backend/src/services/appointment.service.ts`
  - `apps/web/src/pages/client/BookingPage.tsx`
  - `tests/appointment.service.test.ts`
- Exact fix applied:
  - Added backend same-day past-slot filtering in availability service, comparing slot minute-of-day against current UTC time for the requested UTC date key.
  - Added client-side defensive same-day filter in booking page so stale past slots are not shown even if an upstream response is outdated/cached.
  - Added test coverage validating same-day slot filtering behavior in appointment service.
- Tests added/updated:
  - Updated `tests/appointment.service.test.ts` with `filters past same-day slots in availability response`.
- Verification evidence:
  - `npm run lint` ✅
  - `npm run typecheck` ✅
  - `npm run build` ✅
...
- Regressions:
  - None detected in full verification suite.
- Residual limitations:
  - Real-device timezone perception in Telegram WebView still requires final device validation.
...
- Root cause:
  - Calendar grid had no appointment density signal; schedule page fetched appointments but never mapped them to day-level markers in calendar cells.
- Files changed:
  - `apps/web/src/components/calendar/CalendarGrid.tsx`
  - `apps/web/src/pages/lawyer/SchedulePage.tsx`
  - `apps/web/src/lib/appointments.ts`
  - `tests/appointment-calendar-visibility.test.ts`
- Exact fix applied:
  - Added date highlight support to shared `CalendarGrid` (`highlightedDates`) with visual marker + accent border for dates containing bookings.
  - Added schedule-page derivation of highlighted date keys from appointment list and wired it into the calendar.
  - Added deterministic helper to include relevant statuses (`PENDING`, `CONFIRMED`, `AWAITING_CLIENT_RESPONSE`, `COMPLETED`) and ignore non-highlight states.
  - Added unit test coverage for date-key extraction, deduplication, and status filtering.
- Tests added/updated:
  - Added `tests/appointment-calendar-visibility.test.ts`.
- Verification evidence:
  - `npm run lint` ✅
  - `npm run typecheck` ✅
  - `npm run build` ✅
...
- Regressions:
  - None detected in full verification suite.
- Residual limitations:
  - Real-device Telegram rendering/performance of highlighted calendar cells still requires final mobile verification.
